...
trigger() {
	...
	touch $included_subdomains_file && wc $included_subdomains_file
...
if [[ $target_id == "" ]] || [ ! $target_id -eq $target_id ]; then
	echo "ERROR: Could not get Target ID" >&2
	exit 1
fi

get_engine_id $1 "$5"
echo "DEBUG: Engine ID: $engine_id"
if [[ $engine_id == "" ]] || [ ! $engine_id -eq $engine_id ]; then
	echo "ERROR: Could not get Engine ID" >&2
	exit 1
fi
...
echo "$RENGINE_URL/scan/detail/$last_scan_id"
