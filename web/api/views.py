...
class ListSubdomains(APIView):
    def get(self, request, format=None):
        ...
        if scan_id:
            ...
            subdomain_query = Subdomain.objects.filter(
                target_domain__name=target_name
            ).distinct("name")
        else:
...
class IpAddressViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if scan_id:
            ...
            self.queryset = (
                Subdomain.objects.filter(target_domain__name=target_name)
                .exclude(ip_addresses__isnull=True)
                .distinct()
            )
        else:
            self.queryset = (
                Subdomain.objects.all().exclude(ip_addresses__isnull=True).distinct()
            )
        ...
        if port:
            self.queryset = (
                ...
                .exclude(ip_addresses__isnull=True)
                .filter(ip_addresses__ports__in=Port.objects.filter(number=port))
                .distinct()
            )
...
class SubdomainsViewSet(viewsets.ModelViewSet):
    queryset = Subdomain.objects.none()
    serializer_class = OnlyScreenshotsSubdomainSerializer
    ...
    def get_queryset(self):
        ...
        if scan_id:
            ...
            return Subdomain.objects.filter(target_domain__name=target_name).distinct(
                "name"
            )
        else:
...
class EndPointViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if scan_id:
            self.queryset = EndPoint.objects.filter(scan_history__id=scan_id).distinct()
...
def scanStatus(request, scanId):
    ...
    return http.JsonResponse({"error": "false", "scanStatus": scan.scan_status})
